...
- `rd-agent-core`：首发候选版 `0.1.2`，提供 host-neutral turn/run kernel、AgentRunner facade、provider LLM client glue、业务 Agent adapter 协议、事件写入 facade、运行限制/重复工具调用与工具安全策略；只依赖 contracts 与 llm adapter，不依赖 SaaS DB / 前端 / PPT 业务实现。
